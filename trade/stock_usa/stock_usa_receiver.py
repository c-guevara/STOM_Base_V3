...
class StockUsaReceiver(BaseReceiver):
    ...
    def _get_code_info(self):
        dict_info, self.codes = self.ls.get_code_info_stock_usa()
    ...
    def _convert_real_data(self, data):
        ...
        if tr_cd == self.tr_cd_hoga:
            try:
                dt = int(f"{self.str_today}{body['loctime']}")
            ...
            try:
                dt = int(f"{self.str_today}{body['trdtm']}")
                if self.dict_set['전략종료시간'] < int(str(dt)[8:]):
                    return
                code = body['symbol']
                c    = float(body['price'])
                ...
                dm   = int(body['amount'])
                cg   = body['cgubun']
                self.update_tick_data(dt, code, c, o, h, low, v, per, dm, cg)
            except:
